...
/* TRI LOGEMENT PAR TYPE DE LOGEMENT */
void logement10 (void)
{
	Logement **tab;
	int nbLog,taillemax,val,i;
	tab=chargeLog(&nbLog, &taillemax); 
	triDichLogTYP(tab,nbLog);
	val=rechDichLogTYP(0,tab,nbLog);
	afficherToutLog(tab,nbLog);
	for (i=0;i<nbLog;i++)
		free(tab[i]);
...
/* TRI LOGEMENT PAR DISPONIBILITE DU LOGEMENT */
void logement11 (void)
{
	Logement **tab;
	int nbLog,taillemax,val,i;
	tab=chargeLog(&nbLog, &taillemax); 
	triDichLogDIS(tab,nbLog);
	val=rechDichLogDIS(0,tab,nbLog);
	afficherToutLog(tab,nbLog);
	for (i=0;i<nbLog;i++)
		free(tab[i]);
