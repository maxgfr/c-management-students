...
/* CHERCHE ETUDIANTS HANDICAPES*/
void etudiant10 (void)
{
	Etudiant **tab;
	int nbEtu,tailleMax,val,i;
	tab=chargeEtu("etudiant.txt",&nbEtu,&tailleMax);
	afficherToutEtu(tab,nbEtu);
	printf("\n");
	triDichEtuECH(tab,nbEtu);
	afficherToutEtu(tab,nbEtu);
	val=rechDichEtuECH(0,tab,nbEtu);
	if (tab[val]->bourse==0)
		printf("L'étudiant ayant pour id : %s. Il s'appelle %s %s et a pour indice %d dans le tableau.\n",tab[val]->idetud,tab[val]->prenom,tab[val]->nom,val);
	for (i=0;i<tailleMax;i++) 
		free(tab[i]);
}
